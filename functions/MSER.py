...
def paint_acceptable_growth(acceptable_growth,region_map, Width, paint_map, size_min, size_max):
    ...
    for key, value in acceptable_growth.items():
        ...
        if (((1-int(region_map[2, j, i])) >= size_min) and ((1-int(region_map[2, j, i])) < size_max)):
            for num_elem in range(1-int(region_map[2, j, i])):                                                       #loop que muda a seed de todos os px da regiao sendo eliminada
...
def preprocessing(Height, Width, im, region_map):
    ...
    for i in range(Height):                                                    #troca os valore RGB
    ...
    for i in histogram:                                                         #histograma culmulativo
        ...
        culm = culm + i
        index = index + 1
    for i in range(Height):                                                    #organiza o vetor de posicoes
        for j in range(Width):
            px = im[i,j]
...
def union_find(i, j, Width, size_min, size_max, region_map, seed_list, x, y, f1, f2):
        ...
        if (region_map[1, j+y, i+x]):    #se vizinho jah foi visitado
            if (region_map[1, j, i]):     #se pixel atual tem seed:
                ...
                if (region_map[2, j_seed, i_seed] >= region_map[2, j_seed_viz, i_seed_viz]):#se seed da regiao atual menor ou igual regiao vizinha:
                    ...
                    for num_elem in range(size):                                                           #loop que muda a seed de todos os px da regiao sendo eliminada
    ...
    #     absorb = region_map[3, :, :]
        # for num_elem in range(1-int(region_map[2, j_teste, i_teste])):                                                       #loop que muda a seed de todos os px da regiao sendo eliminada
        #     i_temp = get_coordinate(valor, Width)[0]
        #     j_temp = get_coordinate(valor, Width)[1]
        #     valor = region_map[3, j_temp, i_temp]
...
def main(argv):
    path = '/home/sun/Cloud/paint-relief/pics/'
    image_name = "3"
    im = cv2.imread(str(path)+str(image_name) + ".png")
    f1 = open('f1', 'w+')
    f2 = open('f2', 'w+')
    ...
    for num_pix in histogram:                                                   #itera pelas intensidades
        for a in range(int(num_pix)):                                          #marca pixeis dessa intensidade na camada visivel (PX)
            i = int(position_vector[int(a+b)]%Width)
            j = int((position_vector[int(a+b)]-i)/Width)
            region_map[0, j, i] = 0
        #checa se encontra regioes ja criadas
        for a in range(int(num_pix)):                                          #itera pelos pixeis adicionados nessa intensidade
            i = int(position_vector[int(a+b)]%Width)
            j = int((position_vector[int(a+b)]-i)/Width)
